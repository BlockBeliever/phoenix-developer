<template>
  <div>
    <img src="" alt="">
    <!-- list-type上传 -->
    <el-upload v-if="type === 'listType'" action="" :class="{ hide: hideUpload }" list-type="picture-card"
      :http-request="avatarUpload" :limit="limit" :file-list="fileList" :accept="accept" multiple
      :on-success="handleSuccess" :on-change="handleChange">
      <div slot="file" slot-scope="{file}">
        <img v-for="item in fileList" class="el-upload-list__item-thumbnail" :src="item.base64Url" alt="" :key="item">
    ...
    <!-- 拖拽上传 -->
    <el-upload v-if="type === 'drag'" class="upload-demo" drag multiple :http-request="dragUpload">
      <el-icon class="el-icon--upload"><upload-filled /></el-icon>
      <div class="el-upload__text">
        请将上传文件拖至此处 <em>点击上传</em>
      </div>
      <template #tip>
    ...
    <div v-if="type === 'button'">
      <input :id="id" type="file" style="display: none;" name="single" multiple :accept="accept" @change="selectdFile" />
    ...
    async upload(file) {
      ...
      try {
        ...
        cos.uploadFile({
          ...
          Key: `${this.uploadPath}phoenixadmin/${(new Date()).toLocaleDateString()}/${md5(newFile.name.substring(0, index + 1))}${Date.now()}.${newFile.name.substring(index + 1, newFile.name.length)}`, /* 存储在桶里的对象键（例如1.jpg，a/b/test.txt），必须字段 */
          Body: newFile,
          SliceSize: 1024 * 1024 * 300, /* 触发分块上传的阈值，超过300MB 使用分块上传，小于300MB使用简单上传。可自行设置，非必须 */
